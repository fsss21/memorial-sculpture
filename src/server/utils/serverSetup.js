...
/**
 * Конфигурация сервера — проект «Классический Петербург / Мемориальная скульптура» (memorial-sculpture)
 * React + Vite, данные: catalogItems.json, progressPoints.json, progressPointsSections.json в public/data/
 */
const CONFIG = {
  ...
  // Доп. данные секций (MainSection → SubMenu)
  sectionsFile: path.join('data', 'progressPointsSections.json'),
  // Файл голосов (если понадобится API; не используется фронтом)
...
class ServerSetup {
  constructor() {
    try {
      // Корень проекта memorial-sculpture: __dirname = src/server/utils → вверх 3 уровня
      ...
      if (!this.config.gameItemsFile || !this.config.statisticsFile) {
        throw new Error(`CONFIG не инициализирован. gameItemsFile: ${this.config.gameItemsFile}, statisticsFile: ${this.config.statisticsFile}`);
      }

      // Директория со сборкой: Vite → build/
      // В pkg: раздаём статику из текущей рабочей директории (откуда запустили exe),
      // чтобы index.html искался в папке, где лежит exe или куда пользователь перешёл (cd build).
      if (this.isPkg) {
  ...
  async checkIndexHtml() {
    try {
      ...
      if (!exists) {
        ...
        console.log('   Чтобы приложение работало: выполните "npm run build", затем скопируйте');
        console.log('   из папки build файлы index.html и папку assets в папку с launch.exe.\n');
      } else {
  ...
  async initializeDataDir() {
    try {
      ...
      console.log(`📂 catalogItems.json: ${gameItemsExists ? '✅' : '❌'} ${gameItemsFile}`);
      console.log(`📂 progressPoints.json: ${statisticsExists ? '✅' : '❌'} ${statisticsFile}`);
      ...
      if (this.sectionsFileFallback || this.sectionsFile) {
        ...
        if (sectionsPath) {
          ...
          console.log(`📂 progressPointsSections.json: ${sectionsExists ? '✅' : '❌'} ${sectionsPath}`);
        }
      }
...
<body>
  ...
  <ol>
    ...
    <li>Скопируйте из папки <code>build</code> файл <code>index.html</code> и папку <code>assets</code> в ту же папку, где лежит <code>launch.exe</code>.</li>
    <li>Обновите эту страницу (F5).</li>
  </ol>
  <p>Либо выполните полную сборку из корня проекта: <code>npm run build:win</code>, затем запускайте <code>launch.exe</code> из папки <code>build\\</code>.</p>
